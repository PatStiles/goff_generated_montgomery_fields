...
//
// fp are assumed to be in Montgomery form in all methods.
//
// Modulus q =
//
...
const (
	Limbs = 4         // number of 64 bits words needed to represent a fp
	Bits  = 255       // number of bits needed to represent a fp
	Bytes = Limbs * 8 // number of bytes needed to represent a fp
)
...
// Newfp returns a new fp from a uint64 value
//
// it is equivalent to
//
//	var v fp
...
// SetInterface converts provided interface into fp
...
//	big.Int
//	[]byte
func (z *fp) SetInterface(i1 interface{}) (*fp, error) {
	if i1 == nil {
		return nil, errors.New("can't set main.fp with <nil>")
	}
	...
	default:
		return nil, errors.New("can't set main.fp from type " + reflect.TypeOf(i1).String())
	}
}
...
func (z *fp) NotEqual(x *fp) uint64 {
...
func (z *fp) smallerThanModulus() bool {
...
//export cMul
func (z *C.fp) cMul(x, y *C.fp) *C.fp {
...
// Add z = x + y (mod q)
//export cAdd
func (z *C.fp) cAdd(x, y *C.fp) *C.fp {
  var e, f, g fp
  e.cSet(z)
  f.cSet(x)
  g.cSet(y)
...
// Sub z = x - y (mod q)
//export cSub
func (z *C.fp) cSub(x, y *C.fp) *C.fp {
  var e, f, g fp
  e.cSet(z)
  f.cSet(x)
  g.cSet(y)
...
func (z *fp) Select(c int, x0 *fp, x1 *fp) *fp {
...
// Exp z = xᵏ (mod q)
func (z *fp) Exp(x fp, k *big.Int) *fp {
...
func (z *fp) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *fp) ToBigInt(res *big.Int) *big.Int {
	var b [Limbs * 8]byte
...
func (z fp) ToBigIntRegular(res *big.Int) *big.Int {
	z.FromMont()
	return z.ToBigInt(res)
}

// Bytes returns the value of z as a big-endian byte array
func (z *fp) Bytes() (res [Limbs * 8]byte) {
	_z := z.ToRegular()
...
func (z *fp) SetString(number string) (*fp, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("fp.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *fp) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See fp.SetString for valid prefixes (0x, 0b, ...)
func (z *fp) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = fp.Bits * 3)")
	}
...
var (
	_bLegendreExponentfp *big.Int
	_bSqrtExponentfp     *big.Int
)

func init() {
	_bLegendreExponentfp, _ = new(big.Int).SetString("39f6d3a994cebea4199cec0404d0ec02a9ded2017fff2dff7fffffff80000000", 16)
	const sqrtExponentfp = "39f6d3a994cebea4199cec0404d0ec02a9ded2017fff2dff7fffffff"
	_bSqrtExponentfp, _ = new(big.Int).SetString(sqrtExponentfp, 16)
}
...
//export cInverse
func (z *C.fp) cInverse(x *C.fp) *C.fp {
...
func approximate(x *fp, nBits int) uint64 {
...
// 0 ≤ x, y < 2²⁵⁵
// |xC|, |yC| < 2⁶³
func (z *fp) linearComb(x *fp, xC int64, y *fp, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2²⁵⁵ = 2³¹⁹
	// therefore | hi | < 2⁶³ ≤ 2⁶³
...
func (z *fp) montReduceSigned(x *fp, xHi uint64) {
...
func negL(x *fp, xHi uint64) uint64 {
	var b uint64
...
func (z *fp) mulWNonModular(x *fp, y int64) uint64 {
...
func (z *fp) linearCombNonModular(x *fp, xC int64, y *fp, yC int64) uint64 {
	var yTimes fp
