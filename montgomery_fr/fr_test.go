...
func BenchmarkFrSelect(b *testing.B) {
	var x, y fr
	x.SetRandom()
	y.SetRandom()
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		benchResfr.Select(i%3, &x, &y)
	}
}

func BenchmarkFrSetRandom(b *testing.B) {
	var x fr
	x.SetRandom()
...
func BenchmarkFrSetBytes(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		benchResfr.SetBytes(bb[:])
	}

}

func BenchmarkFrMulByConstants(b *testing.B) {
	b.Run("mulBy3", func(b *testing.B) {
		benchResfr.SetRandom()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			MulBy3(&benchResfr)
		}
...
func BenchmarkFrInverse(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		benchResfr.Inverse(&x)
	}

}

func BenchmarkFrButterfly(b *testing.B) {
	var x fr
	x.SetRandom()
	benchResfr.SetRandom()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		Butterfly(&x, &benchResfr)
	}
}
...
func BenchmarkFrDouble(b *testing.B) {
	benchResfr.SetRandom()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		benchResfr.Double(&benchResfr)
	}
}
...
func BenchmarkFrAdd(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		benchResfr.Add(&x, &benchResfr)
	}
}

func BenchmarkFrSub(b *testing.B) {
	var x fr
	x.SetRandom()
	benchResfr.SetRandom()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		benchResfr.Sub(&x, &benchResfr)
	}
}

func BenchmarkFrNeg(b *testing.B) {
	benchResfr.SetRandom()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		benchResfr.Neg(&benchResfr)
	}
}

func BenchmarkFrDiv(b *testing.B) {
	var x fr
	x.SetRandom()
	benchResfr.SetRandom()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		benchResfr.Div(&x, &benchResfr)
	}
}

func BenchmarkFrFromMont(b *testing.B) {
	benchResfr.SetRandom()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		benchResfr.FromMont()
...
func BenchmarkFrToMont(b *testing.B) {
	benchResfr.SetRandom()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		benchResfr.ToMont()
...
func BenchmarkFrSquare(b *testing.B) {
	benchResfr.SetRandom()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		benchResfr.Square(&benchResfr)
	}
}

func BenchmarkFrSqrt(b *testing.B) {
	var a fr
	a.SetUint64(4)
	a.Neg(&a)
	b.ResetTimer()
...
func BenchmarkFrMul(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		benchResfr.Mul(&benchResfr, &x)
	}
}

func BenchmarkFrCmp(b *testing.B) {
	x := fr{
		15831548891076708299,
		4682191799977818424,
		12294384630081346794,
...
func TestFrIsRandom(t *testing.T) {
	for i := 0; i < 50; i++ {
		var x, y fr
		x.SetRandom()
		y.SetRandom()
...
func TestFrIsUint64(t *testing.T) {
	t.Parallel()
...
func TestFrNegZero(t *testing.T) {
	var a, b fr
	b.SetZero()
	for a.IsZero() {
		a.SetRandom()
...
var staticTestValues []fr

func init() {
	staticTestValues = append(staticTestValues, fr{})    // zero
	...
	staticTestValues = append(staticTestValues, fr{0})
	staticTestValues = append(staticTestValues, fr{0, 0})
	staticTestValues = append(staticTestValues, fr{1})
	staticTestValues = append(staticTestValues, fr{0, 1})
	staticTestValues = append(staticTestValues, fr{2})
	staticTestValues = append(staticTestValues, fr{0, 2})
...
func TestFrReduce(t *testing.T) {
	testValues := make([]fr, len(staticTestValues))
...
func TestFrEqual(t *testing.T) {
	...
	properties.Property("x.Equal(&y) iff x == y; likely false for random pairs", prop.ForAll(
		func(a testPairfr, b testPairfr) bool {
...
func TestFrInverseExp(t *testing.T) {
	// inverse must be equal to exp^-2
	exp := Modulus()
	exp.Sub(exp, new(big.Int).SetUint64(2))

	invMatchExp := func(a testPairfr) bool {
		var b fr
		b.Set(&a.element)
	...
	properties = gopter.NewProperties(parameters)
	properties.Property("inv(0) == 0", prop.ForAll(invMatchExp, ggen.OneConstOf(testPairfr{})))
	properties.TestingRun(t, gopter.ConsoleReporter(false))

}

func mulByConstant(z *fr, c uint8) {
	var y fr
	y.SetUint64(uint64(c))
	z.Mul(z, &y)
}

func TestFrMulByConstants(t *testing.T) {
...
func TestFrLegendre(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	if testing.Short() {
		parameters.MinSuccessfulTests = nbFuzzShort
...
func TestFrButterflies(t *testing.T) {
...
func TestFrLexicographicallyLargest(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	if testing.Short() {
		parameters.MinSuccessfulTests = nbFuzzShort
...
func TestFrSelect(t *testing.T) {
	...
	properties.Property("Select: must select correctly", prop.ForAll(
		func(a, b fr, cond int64, z int8) bool {
...
func TestFrSetInt64(t *testing.T) {
	...
	properties.Property("z.SetInt64 must match z.SetString", prop.ForAll(
		func(a testPairfr, v int64) bool {
			c := a.element
			d := a.element
...
func TestFrSetInterface(t *testing.T) {
	...
	properties.Property("z.SetInterface must match z.SetString with int16", prop.ForAll(
		func(a testPairfr, v int16) bool {
			c := a.element
			d := a.element
	...
	properties.Property("z.SetInterface must match z.SetString with int32", prop.ForAll(
		func(a testPairfr, v int32) bool {
			c := a.element
			d := a.element
	...
	properties.Property("z.SetInterface must match z.SetString with uint8", prop.ForAll(
		func(a testPairfr, v uint8) bool {
			c := a.element
			d := a.element
	...
	properties.Property("z.SetInterface must match z.SetString with uint16", prop.ForAll(
		func(a testPairfr, v uint16) bool {
			c := a.element
			d := a.element
	...
	properties.Property("z.SetInterface must match z.SetString with uint32", prop.ForAll(
		func(a testPairfr, v uint32) bool {
			c := a.element
			d := a.element
	...
	properties.Property("z.SetInterface must match z.SetString with uint64", prop.ForAll(
		func(a testPairfr, v uint64) bool {
			c := a.element
			d := a.element
...
func TestFrNegativeExp(t *testing.T) {
	t.Parallel()
...
func TestFrBatchInvert(t *testing.T) {
	...
	properties.Property("batchInvert --> x * x⁻¹ == 1", prop.ForAll(
		func(tp testPairfr, r uint8) bool {
...
func TestFrFromMont(t *testing.T) {
...
func genFull() gopter.Gen {
	return func(genParams *gopter.GenParameters) *gopter.GenResult {
		...
		var carry uint64
		a[0], carry = bits.Add64(a[0], qfr[0], carry)
		a[1], carry = bits.Add64(a[1], qfr[1], carry)
		a[2], carry = bits.Add64(a[2], qfr[2], carry)
		a[3], _ = bits.Add64(a[3], qfr[3], carry)
...
func (z *fr) matchVeryBigInt(aHi uint64, aInt *big.Int) error {
	var modulus big.Int
	var aIntMod big.Int
	modulus.SetInt64(1)
...
func (z *fr) assertMatchVeryBigInt(t *testing.T, aHi uint64, aInt *big.Int) {
...
func TestfrInversionApproximation(t *testing.T) {
	var x fr
	for i := 0; i < 1000; i++ {
		x.SetRandom()
...
func TestfrInversionCorrectionFactorFormula(t *testing.T) {
...
func TestfrLinearComb(t *testing.T) {
	var x fr
	var y fr
...
func TestfrInversionCorrectionFactor(t *testing.T) {
...
func TestfrBigNumNeg(t *testing.T) {
	var a fr
	aHi := negL(&a, 0)
	if !a.IsZero() || aHi != 0 {
		t.Error("-0 != 0")
...
func TestfrBigNumWMul(t *testing.T) {
	var x fr
...
func TestfrVeryBigIntConversion(t *testing.T) {
	xHi := mrand.Uint64()
	var x fr
	x.SetRandom()
	var xInt big.Int
...
func TestfrMontReduce(t *testing.T) {
...
func TestfrMontReduceMultipleOfR(t *testing.T) {
...
func Testfr0Inverse(t *testing.T) {
	var x fr
	x.Inverse(&x)
	if !x.IsZero() {
		t.Fail()
...
func testLinearComb(t *testing.T, x *fr, xC int64, y *fr, yC int64) {
...
func testBigNumWMul(t *testing.T, a *fr, c int64) {
	var aHi uint64
	var aTimes fr
...
func (z *fr) toVeryBigIntUnsigned(i *big.Int, xHi uint64) {
	z.ToBigInt(i)
...
func (z *fr) toVeryBigIntSigned(i *big.Int, xHi uint64) {
...
func assertMulProduct(t *testing.T, x *fr, c int64, result *fr, resultHi uint64) big.Int {
	var xInt big.Int
	x.ToBigInt(&xInt)
...
func approximateRef(x *fr) uint64 {
