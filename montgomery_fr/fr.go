...
const (
	Limbs = 4         // number of 64 bits words needed to represent a fr
	Bits  = 253       // number of bits needed to represent a fr
	Bytes = Limbs * 8 // number of bytes needed to represent a fr
)
...
// Newfr returns a new fr from a uint64 value
//
// it is equivalent to
//
//	var v fr
...
// SetInterface converts provided interface into fr
...
func (z *fr) SetInterface(i1 interface{}) (*fr, error) {
	if i1 == nil {
		return nil, errors.New("can't set main.fr with <nil>")
	}
	...
	default:
		return nil, errors.New("can't set main.fr from type " + reflect.TypeOf(i1).String())
	}
}
...
func (z *fr) NotEqual(x *fr) uint64 {
...
func (z *fr) smallerThanModulus() bool {
...
func (z *fr) ToBigInt(res *big.Int) *big.Int {
	var b [Limbs * 8]byte
...
func (z *fr) Bytes() (res [Limbs * 8]byte) {
	_z := z.ToRegular()
...
func (z *fr) SetString(number string) (*fr, error) {
	...
	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("fr.SetString failed -> can't parse number into a big.Int " + number)
	}
...
// See fr.SetString for valid prefixes (0x, 0b, ...)
func (z *fr) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = fr.Bits * 3)")
	}
...
var (
	_bLegendreExponentfr *big.Int
	_bSqrtExponentfr     *big.Int
)

func init() {
	_bLegendreExponentfr, _ = new(big.Int).SetString("e7db4ea6533afa906673b0101343b007fc7c3803a0c8238ba7e835a943b73f0", 16)
	const sqrtExponentfr = "73eda753299d7d483339d80809a1d803fe3e1c01d06411c5d3f41ad4a1db9f"
	_bSqrtExponentfr, _ = new(big.Int).SetString(sqrtExponentfr, 16)
}
...
func approximate(x *fr, nBits int) uint64 {
...
// 0 ≤ x, y < 2²⁵³
// |xC|, |yC| < 2⁶³
func (z *fr) linearComb(x *fr, xC int64, y *fr, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2²⁵³ = 2³¹⁷
	// therefore | hi | < 2⁶¹ ≤ 2⁶³
...
func (z *fr) montReduceSigned(x *fr, xHi uint64) {
...
func negL(x *fr, xHi uint64) uint64 {
	var b uint64
...
func (z *fr) mulWNonModular(x *fr, y int64) uint64 {
...
func (z *fr) linearCombNonModular(x *fr, xC int64, y *fr, yC int64) uint64 {
	var yTimes fr
